...
TOKEN_LENGTH = 2
INFO_HASH_LEN = 500000 #50w数据，限制内存不至于消耗太大
CACHE_LEN = 100 #更新数据库缓存
WAIT_DOWNLOAD = 80 #等待下载数量
...
class Master(Thread): #解析info_hash
    ...
    def start_work(self,max):
        for item in xrange(max):
            t = threading.Thread(target=self.work,args=(item,)) #开启线程工作
            t.setDaemon(True)
            t.start()
    ...
    def prepare_download_metadata(self):
        if self.queue.qsize() == 0:                 #如果队列为空则等等2秒
            sleep(2)
        address ,binhash = self.queue.get()         #获取队列数据
        if binhash in self.visited:                 #去除已经爬取的资源
            return
        if len(self.visited) > 100000:              #如果爬取的资源超过10万 然后就置空 重新计数
            self.visited = set()
    ...
    def download_metadata(self):                    #下载源数据
        if self.cache.qsize() > CACHE_LEN/2:        #出队更新下载
            while self.cache.qsize() > 0:           #当缓存队列大于0时
                address,binhash,utcnow = self.cache.get()
                info_hash = binhash.encode('hex')
                self.dbcurr.execute('SELECT id FROM test_hash WHERE info_hash=%s', (info_hash,))    #查看是否已经拥有该资源，如果有了 则更新 last_seen时间
                y = self.dbcurr.fetchone()
                if y:
                    self.dbcurr.execute('UPDATE test_hash SET last_seen=%s, requests=requests+1 WHERE info_hash=%s',(utcnow, info_hash))
                else:
                    self.waitDownload.put((address,binhash))    #没有查找到该资源 则下载该资源
            self.dbconn.commit()
            if self.waitDownload.qsize() > WAIT_DOWNLOAD:       #如果等待下载资源大于 80 个 则下载
    ...
    def decode_utf8(self,d,i):
        if i+'.utf-8' in d:
            return d[i+'.utf-8'].encoding('utf8')
    ...
    def got_torrent(self):
        if self.metadata_queue.qsize() == 0:
            return
        binhash,address,data,start_time = self.metadata_queue.get()     #队列获取资源
        if not data:
            return
        try:
        ...
        utcnow = time.strftime("%Y-%m-%d %H:%M:%S",x) #get now  time
        ...
        try:
            ...
            ret = self.dbcurr.execute('INSERT INTO test_hash(info_hash,category,data_hash,name,extension,classified,source_ip,tagged,' +
